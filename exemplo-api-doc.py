def contrato_dependencia(matricula_id):
    import json
    import os.path
    import io
    import pickle
    ...
    new_file = drive_service.files().copy(
        body={
            ...
            'name':'Contrato DEPENDÊNCIA ' + str(pessoa.id+'_'+str(matricula_id)),
    ...
    #https://docs.google.com/document/d/{{doc_id}}/pub?embedded=true
    ...
    return dict(doc_id=new_file.get('id'))


def get_replace_field(string_field, value):
    c = {
        'replaceAllText': {
            'containsText': {
                'text': '{{'+string_field+'}}',
